...
for folder in "$base_dir"/*; do
    if [ -d "$folder" ]; then
        ...
        if [[ ! -f "$counts_path" || ! -f "$markers_path" || ! -f "$K_path" ]]; then
            ...
            python /scratch/lalonsoeste/PhD/NMF_deconvolution/results_NB/benchmarking/DLPFC/process_data.py "$folder"
        ...
        bash /scratch/lalonsoeste/PhD/NMF_deconvolution/results_NB/benchmarking/benchmark.sh \
            ...
            --output_path="/scratch/lalonsoeste/PhD/NMF_deconvolution/results_NB/benchmarking/DLPFC/results/${sample}" \
            --visium=true \
            --k="$k" \
            --starfysh_lr="1e-6" \
            --hungarian=false &
...
for folder in "$base_dir"/*; do
    if [ -d "$folder" ]; then
        ...
        python /scratch/lalonsoeste/PhD/NMF_deconvolution/results_NB/benchmarking/DLPFC/annotate.py \
            "${folder}/filtered_feature_bc_matrix.h5ad" \
            "/scratch/lalonsoeste/PhD/NMF_deconvolution/results_NB/benchmarking/DLPFC/results/${sample}"
    fi
done

python /scratch/lalonsoeste/PhD/NMF_deconvolution/results_NB/benchmarking/DLPFC/experiment_results.py \
    /scratch/lalonsoeste/PhD/NMF_deconvolution/results_NB/benchmarking/DLPFC/results/
