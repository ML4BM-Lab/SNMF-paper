...
# --- Paths -----------------------------------------------------------------
marker_genes_path <- "/scratch/lalonsoeste/PhD/NMF_deconvolution/data/TNBC/processed/marker_genes.csv"
signatures_path   <- "/scratch/lalonsoeste/PhD/NMF_deconvolution/results/benchmarking/TNBC/SNMF/signatures.csv"

# Create output directory
out_dir <- "/scratch/lalonsoeste/PhD/NMF_deconvolution/results/benchmarking/TNBC/SNMF/signatures_analysis/figures"
...
top_genes_per_sig <- apply(signatures_mat, 2, function(x) {
  head(names(sort(x, decreasing = TRUE)), 10)
}) %>% unlist() %>% unique()

plot_df <- signatures_mat[top_genes_per_sig, , drop = FALSE] %>%
  as.data.frame() %>%
  rownames_to_column("gene") %>%
  pivot_longer(-gene, names_to = "Signature", values_to = "Weight")
